...
<?php 
    ...
    while ($statusmodalrow = mysqli_fetch_assoc($statusmodalresult)) {
        ...
        $tstatus = $orderStatuses[$status] ?? "Order Cancelled.";
        
        // Fetch delivery details if applicable
        if ($status >= 1 && $status <= 4) {
            ...
            $trackId = $deliveryDetailRow['id'] ?? 'xxxxx';
            $deliveryBoyName = $deliveryDetailRow['deliveryBoyName'] ?? 'N/A';
            $deliveryBoyPhoneNo = $deliveryDetailRow['deliveryBoyPhoneNo'] ?? 'N/A';
            $deliveryTime = $deliveryDetailRow['deliveryTime'] ?? 'xx';
        } else {
            $trackId = 'xxxxx';
            $deliveryBoyName = 'N/A';
